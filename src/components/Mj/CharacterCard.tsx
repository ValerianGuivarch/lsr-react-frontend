        ...
        <CardContainer selected={props.selected} url={props.character.picture}>
            <CardContent>
                <TitleContainer allie={props.allie}>
                    <ProfilePicture src={props.character.picture}
                                    alt="Profile Picture"
                                    onClick={() => props.onSelect(props.character.name)}
                                    selected={props.selected}
                    />
                    <CharacterName  onClick={() => props.onSelect(props.character.name)} >{props.character.name}</CharacterName>
                    <IconWrapper onClick={
                        () => {
                            window.location.href = `/characters/${props.character.name}`;
                        }}>
                        <FaIdBadge size={14}/></IconWrapper>
                    ...
                    {props.onDelete && <IconWrapper onClick={() => props.onDelete && props.onDelete(props.character.name)}><FaTrashAlt size={14}/></IconWrapper>}
                </TitleContainer>
                <CharacterPanelCard>
                    <CharacterPanel
...
const CardContainer = styled.div<{ selected: boolean, url?: string }>`
  ...
  &::before {
    ...
    background-color: ${(props) => props.selected ? 'rgba(128, 128, 128, 0.5)' : 'rgba(255, 255, 255, 0.7)'};
