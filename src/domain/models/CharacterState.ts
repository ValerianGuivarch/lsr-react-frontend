...
export class CharacterState {
    ...
    constructor(p :{state?: CharacterState, character?: Character}) {
        this.focusActivated = (p.state && p.character?.pf && p.character?.pf > 0) ? p.state.focusActivated : false;
        this.powerActivated = (p.state && p.character?.pp && p.character?.pp > 0) ? p.state.powerActivated : false;
        ...
        this.proficiencies = p.state ? p.state.proficiencies : new Map<string, boolean>();
